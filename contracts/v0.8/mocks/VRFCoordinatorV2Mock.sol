...
contract VRFCoordinatorV2Mock is VRFCoordinatorV2Interface {
  ...
  uint256 public immutable GAS_PRICE_WKCS;
  ...
  constructor(
    uint256 _baseFee,
    uint256 _gasPriceWKCS,
    bool _paymentEnabled
  ) {
    BASE_FEE = _baseFee;
    GAS_PRICE_WKCS = _gasPriceWKCS;
    s_paymentEnabled = _paymentEnabled;
  }
   ...
   * @dev against the real coordinator a small amount of additional WKCS is required.
   *
  ...
  function fulfillRandomWordsWithOverride(
    ...
    uint256 payment = uint256(BASE_FEE + ((startGas - gasleft()) * GAS_PRICE_WKCS));
  ...
  {
    return (4, 2_500_000, 33285, s_paymentEnabled, 100000);
  }
