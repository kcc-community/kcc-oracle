...
describe('VRFCoordinatorV2Mock', () => {
  ...
  let pointOneWrapperNative = BigNumber.from('100000000000000000')
  let oneWrapperNative = BigNumber.from('1000000000000000000')
  let keyhash =
  ...
  beforeEach(async () => {
    ...
    const vrfCoordinatorV2MockFactory = await ethers.getContractFactory(
      'contracts/v0.8/mocks/VRFCoordinatorV2Mock.sol:VRFCoordinatorV2Mock',
      accounts[0],
    )
    vrfCoordinatorV2Mock = await vrfCoordinatorV2MockFactory.deploy(
      pointOneWrapperNative,
      1e9, // 0.000000001 native per gas
      true,
    )
    ...
    wrapperToken = await ltFactory.deploy()

    const vrfConsumerV2Factory = await ethers.getContractFactory(
      'contracts/v0.8/tests/VRFConsumerV2.sol:VRFConsumerV2',
      accounts[0],
    )
  ...
  describe('#fundSubscription', async function () {
    it('can fund a subscription', async function () {
      ...
      await expect(
        vrfCoordinatorV2Mock
          .connect(subOwner)
          .fundSubscription(subId, oneWrapperNative),
      )
        .to.emit(vrfCoordinatorV2Mock, 'SubscriptionFunded')
        .withArgs(subId, 0, oneWrapperNative)
      let sub = await vrfCoordinatorV2Mock
        .connect(subOwner)
        .getSubscription(subId)
      expect(sub.balance).to.equal(oneWrapperNative)
    })
    it('cannot fund a nonexistent subscription', async function () {
      await expect(
        vrfCoordinatorV2Mock
          .connect(subOwner)
          .fundSubscription(4, oneWrapperNative),
      ).to.be.revertedWith('InvalidSubscription')
    })
  })
  ...
  describe('#fulfillRandomWords', async function () {
    ...
    it('can request and fulfill [ @skip-coverage ]', async function () {
      ...
      expect(sub.balance).to.equal(
        oneWrapperNative.sub(receipt.events[0].args['payment']),
      )
    })
