...
describe('BlockhashStore', () => {
  ...
  describe('#store', () => {
    it('stores batches of blocknumbers', async () => {
      ...
      const numBlocks = 3
      await blockhashStore
        .connect(owner)
        .batchStore(range(numBlocks, bottomBlock))
    ...
    it('skips block numbers that are too far back', async () => {
      ...
      await blockhashStore.connect(owner).batchStore([1, 2, 3, gettableBlock])
      await ethers.provider.send('evm_mine', [])
    ...
    it('store gt 256 block number', async () => {
      const latestBlock = await ethers.provider.send('eth_blockNumber', [])
      const bottomBlock = latestBlock - 5
      ...
      await expect(
        blockhashStore.connect(user).store(bottomBlock),
      ).to.be.revertedWith('blockhash(n) failed')
    })
  })

  describe('#storeByOwner', () => {
    it('stores batches of blocknumbers by owner', async () => {
      const latestBlock = await ethers.provider.send('eth_blockNumber', [])
      const bottomBlock = latestBlock - 5
      const numBlocks = 3
      ...
      for (let i = bottomBlock; i < bottomBlock + numBlocks; i++) {
        const expectedBh = (await ethers.provider.getBlock(i)).hash
        blockNumbers[k] = i
        blockHashes[k] = expectedBh
        k++
      }

      // the block number must be earlier than the latest 256 blocks
      for (let i = 0; i < 256; i++) {
        await ethers.provider.send('evm_mine', [])
      }
      ...
      await expect(
        blockhashStore
          .connect(owner)
          .batchStoreByOwner(blockNumbers, blockHashes),
      ).to.emit(blockhashStore, 'BlockhashfilledByOwner')
    ...
    it('bad input', async () => {
      ...
      for (let i = bottomBlock; i < bottomBlock + numBlocks; i++) {
        blockNumbers[k] = i
        blockHashes[k] = ethers.utils.formatBytes32String('')
        k++
      }
      ...
      await expect(
        blockhashStore
          .connect(owner)
          .batchStoreByOwner(blockNumbers, blockHashes),
      ).to.be.revertedWith('blockhash error')
    })

    it('store lt 256 block number', async () => {
      const latestBlock = await ethers.provider.send('eth_blockNumber', [])
      const bottomBlock = latestBlock - 5
      const numBlocks = 3
      ...
      await expect(
        blockhashStore
          .connect(owner)
          .storeByOwner(blockNumbers[0], blockHashes[0]),
      ).to.be.revertedWith(
        'the block number must be earlier than the latest 256 blocks',
      )
    })
  ...
  describe('#storeEarliest', () => {
    it('store earliest', async () => {
      const latestBlock = await ethers.provider.send('eth_blockNumber', [])
      ...
      await expect(blockhashStore.connect(user).storeEarliest()).to.emit(
        blockhashStore,
        'Blockhashfilled',
      )

      const actualBh = await blockhashStore.getBlockhash(latestBlock)
      const expectedBh = (await ethers.provider.getBlock(latestBlock)).hash
      expect(expectedBh).to.equal(actualBh)
    })
  })
  ...
  describe('#getBlockhashes', () => {
    it('fetches blockhashes of a batch of block numbers', async () => {
      ...
      const actualBlockhashes = await blockhashStore
        .connect(owner)
    ...
    it('returns 0x0 for block numbers without an associated blockhash', async () => {
      ...
      const bottomBlock = latestBlock - 5
      const numBlocks = 3
      const blockhashes = await blockhashStore
        .connect(owner)
        .getBlockhashes(range(numBlocks, bottomBlock))
      const expected = [
  ...
  describe('#batchStoreVerifyHeader', () => {
    it('stores batches of blocknumbers using batchStoreVerifyHeader [ @skip-coverage ]', async () => {
      // Store a single blockhash and go backwards from there using batchStoreVerifyHeader
      const latestBlock = await ethers.provider.send('eth_blockNumber', [])
      await blockhashStore.connect(owner).batchStore([latestBlock])
      await ethers.provider.send('evm_mine', [])
      ...
      // await blockhashStore
      //   .connect(owner)
      //   .batchStoreVerifyHeader(blockNumbers, blockHeaders)
